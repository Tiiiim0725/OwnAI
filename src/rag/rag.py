...
class RAGSystem:
    ...
    def chunk(
        ...
        if ck is None:
            logger.error(f'failed chunk {table_name}-{data_id}.')
    ...
    def _embed_and_upsert(self, cks: list[Chunk], batch_count: int = 100):
        ...
        for i in range(0, len(cks), batch_count):
            ...
            try:
                ...
                logger.info(f"Embedding: {min(i + batch_count, len(cks))}/{len(cks)} [{now_str}]")
                embeddings = self.embed(descriptions)
                self.vector_store.add(batch, embeddings) # type: ignore
                failed_batches = 0
                now_str = datetime.now().strftime("%Y-%m-%d %H:%M")
                logger.info(f"Finish: {min(i + batch_count, len(cks))}/{len(cks)} [{now_str}]")
            except Exception:
                failed_batches += 1
                logger.exception(f"Failed to process batch {i // batch_count}")
                if failed_batches >= 10:
                    logger.error("Too many consecutive failures, aborting.")
                    break
                continue
        if failed_batches:
            logger.warning(f"Embedding completed with {failed_batches} failed batch(es).")
    ...
    def sync_rag(
        ...
        logger.info(f"Find {len(unsync_data)} data needs vectorized.")
        cks = []
        for ud in unsync_data:
            table = ud['table_name']
            rid = ud['record_id']
            logger.info(f'chunking {table}-{rid}')
            try:
                ck = self.chunk(table, rid)
            except ValueError:
                logger.error(f'skipping {table}-{rid}: chunk not found')
                continue
            cks.append(ck)
    ...
    def clean_zero_rag(
        ...
        logger.info(f"Find {len(zero_points)} data needs re_vectorized.")
        cks = []
